...
export const getReceiptURL = async (orderID) => {
	if (!auth.currentUser) return;
	try {
		return await getDownloadURL(ref(storage, `/users/${auth.currentUser.uid}/orders/${orderID}/receipt`));
	} catch {
		return null;
	}
};
...
export const getbankTransferURL = async (orderID) => {
	if (!auth.currentUser) return;
	try {
		return await getDownloadURL(ref(storage, `/users/${auth.currentUser.uid}/orders/${orderID}/bankTransfer`));
	} catch {
		return null;
	}
};
