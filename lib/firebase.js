...
export const getFiles = async (path, orderID, uid = auth.currentUser.uid) => {
	if (!auth.currentUser) return;
	try {
		const list = await listAll(ref(storage, `/users/${uid}/orders/${orderID}/${path}`));
		const names = list.items.map(({ name }) => name);
		const promises = list.items.map(getDownloadURL);
		const URLS = await Promise.all(promises);
		return URLS.map((URL, index) => {
			return { URL, name: names[index], path };
		});
	} catch {
		return [];
	}
...
export const getICs = async (userID = auth.currentUser.uid) => {
	try {
		...
		const URLS = await Promise.all(promises);
		return URLS.map((URL, index) => {
			return { URL, name: names[index], ref: list.items[index], path: "ICs" };
		});
	} catch (e) {
		console.error(e);
		return [];
...
export const sendNotification = async (
	uid,
	{ title, subtitle, details, type, href, notification: { email, SMS, app, desktop } = { email: true, SMS: true, app: true, desktop: true } }
) => {
	console.log({
		title,
		subtitle,
